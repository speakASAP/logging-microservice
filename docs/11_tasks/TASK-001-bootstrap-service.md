# TASK-001-bootstrap-service: Bootstrap logging-microservice
...
Bring the already-running `logging-microservice` into full IPS project-adoption compliance: a complete `ips-adoption.json` profile and the required root/`docs/` document set, reformatted from its existing real operational documentation without inventing new business intent.

## Upstream links

- `../../BUSINESS.md` — approved centralized-logging business intent.
...
Preserves all invariants in `../17_governance/PROJECT_INVARIANTS.md` (centralized logging purpose, timestamp/duration_ms preservation, no log deletion by AI, file-based rotation architecture, sensitive-data policy, ecosystem-wide review for API changes). This task changes documentation only, not runtime behavior.

## Sensitive-data classification

None. No secrets, tokens, or production log content are included in any onboarding document.
...
- Add the missing `docs/06_architecture/INTEGRATION_CONTRACT.md`, `docs/11_tasks/TASK-001-bootstrap-service.md`, `docs/21_execution_plans/EP-TASK-001-bootstrap-service.md`, `docs/12_validation/VAL-TASK-001-bootstrap-service.md`, and `ips-adoption.json`.
- Reformat `README.md`, `BUSINESS.md`, `AGENTS.md`, `AGENT_OPERATIONS.md`, `TASKS.md`, `STATE.json`, and `docs/17_governance/PROJECT_INVARIANTS.md` into the required IPS section/schema structure, preserving all pre-existing real content.
...
- No change to the service's actual business purpose, SLAs, or integrations beyond documenting them accurately.
