...
impact_level: high
impact_description: Adds traceability, an integration decision matrix, and validation gates for all future logging-microservice changes.
...
Bring `logging-microservice` into full IPS project-adoption compliance, matching the standard already applied to `cv-tuning`, `runlayer`, and `wisdom-quotes`.

## Contribution

This task creates the complete governance/documentation baseline (constitution, vision approval, invariants, integration contract, bootstrap task/plan/validation chain) needed to preserve service intent during future AI-assisted maintenance, reducing the risk of undocumented API drift or unreviewed integration changes.
...
Compatible with all invariants in `../17_governance/PROJECT_INVARIANTS.md` — this task only adds documentation and does not touch runtime code, log retention behavior, or the API surface.
...
```
python3 intent-preservation-system/scripts/validate_adoption_profile.py --root logging-microservice --phase planning
```
